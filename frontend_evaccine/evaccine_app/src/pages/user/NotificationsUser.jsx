...
export default function NotificationsUser() {
  ...
  const fetchNoti = useCallback(async () => {
    try {
      ...
      const sorted = [...list].sort((a, b) => {
        ...
        const xa = da ? new Date(da).getTime() : 0;
        const xb = db ? new Date(db).getTime() : 0;
        return xb - xa;
      });
  ...
  const toggleExpand = (id) => {
   ...
   useEffect(() => { fetchNoti(); }, [fetchNoti]);

  const filtered = useMemo(() => {
    let tmp = items;

    if (filter === "unread") tmp = tmp.filter((i) => !i.is_read);
    if (filter === "read") tmp = tmp.filter((i) => i.is_read);
  ...
  return (
    <div className="tw-min-h-screen tw-bg-cyan-50 tw-flex tw-flex-col tw-mt-[95px] tw-py-8">
      {/* Header */}
      <div className="tw-sticky tw-top-0 tw-z-10 tw-bg-cyan-50/80 tw-backdrop-blur tw-border-b tw-border-slate-200 tw-px-4 md:tw-px-8 tw-py-3 tw-flex tw-items-center tw-justify-between">
        ...
        <div className="tw-flex tw-items-center tw-gap-3">
          <button onClick={refresh} className="tw-flex tw-items-center tw-gap-1 tw-text-[12px] tw-bg-white tw-border tw-border-slate-200 tw-rounded-lg tw-px-3 tw-py-1.5 hover:tw-bg-slate-100">
            <i className={`fa-solid fa-rotate ${refreshing ? "tw-animate-spin" : ""}`} /> Làm mới
          </button>
          <button
            onClick={async () => {
            ...
            className="tw-hidden sm:tw-flex tw-items-center tw-gap-1 tw-text-[12px] tw-bg-emerald-500 tw-text-white tw-rounded-lg tw-px-3 tw-py-1.5 hover:tw-bg-emerald-600"
          >
            <i className="fa-regular fa-circle-check" /> Đánh dấu tất cả đã đọc
          </button>
        </div>
      </div>
      ...
      <div className="tw-px-4 md:tw-px-8 tw-pt-4 tw-pb-2">
        <div className="tw-flex tw-gap-2 tw-flex-wrap tw-items-center">
          <button
            ...
            className={`tw-text-[12px] tw-rounded-full tw-px-3 tw-py-1.5 ${filter === "all" ? "tw-bg-yellow-500 tw-text-white" : "tw-bg-white tw-text-slate-700 tw-border tw-border-slate-200"}`}
          >
            Tất cả ({items.length})
          </button>
          <button
            onClick={() => { setFilter("unread"); setPage(1); }}
            className={`tw-text-[12px] tw-rounded-full tw-px-3 tw-py-1.5 ${filter === "unread" ? "tw-bg-cyan-500 tw-text-white" : "tw-bg-white tw-text-slate-700 tw-border tw-border-slate-200"}`}
          >
            Chưa đọc ({items.filter((i) => !i.is_read).length})
          </button>
          <button
            onClick={() => { setFilter("read"); setPage(1); }}
            className={`tw-text-[12px] tw-rounded-full tw-px-3 tw-py-1.5 ${filter === "read" ? "tw-bg-slate-900/80 tw-text-white" : "tw-bg-white tw-text-slate-700 tw-border tw-border-slate-200"}`}
          >
            Đã đọc ({items.filter((i) => i.is_read).length})
          </button>

          {/* lọc theo ngày */}
          <div className="tw-flex tw-items-center tw-gap-2 tw-ml-auto tw-flex-wrap">
            <span className="tw-text-[11px] tw-text-slate-500">Từ ngày:</span>
            <input
              type="date" value={fromDate}
              onChange={(e) => { setFromDate(e.target.value); setPage(1); }}
              className="tw-text-[11px] tw-border tw-border-slate-200 tw-rounded-lg tw-px-2 tw-py-[3px]"
            />
            <span className="tw-text-[11px] tw-text-slate-500">Đến:</span>
            <input
            ...
            {(fromDate || toDate) && (
              <button type="button"  onClick={() => { setFromDate(""); setToDate(""); setPage(1); }}
                className="tw-text-[11px] tw-bg-red-500 tw-text-white tw-border tw-border-red-500 tw-rounded-full tw-px-2 tw-py-[2px] hover:tw-bg-red-600" >
                Xóa lọc
              </button>
            )}
          </div>

          <span className="tw-text-[11px] tw-text-slate-400 tw-self-center tw-ml-2">{filtered.length} thông báo</span>
        </div>
      </div>
        ...
        {loading ? (
          <div className="tw-space-y-3 tw-mt-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="tw-h-[76px] tw-bg-white tw-rounded-xl tw-border tw-border-slate-100 tw-animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="tw-space-y-3 tw-mt-4">
            {view.map((n) => {
              ...
              return (
                <div key={n.id} className={`tw-rounded-xl tw-border tw-px-4 tw-py-3 tw-bg-white tw-flex tw-gap-3 ${!n.is_read ? "tw-border-emerald-200" : "tw-border-slate-100"}`}>
                  <div className="tw-pt-1">
                    <div className={`tw-w-9 tw-h-9 tw-rounded-full tw-flex tw-items-center tw-justify-center ${!n.is_read ? "tw-bg-emerald-100 tw-text-emerald-700" : "tw-bg-pink-100 tw-text-pink-500"}`}>
                  ...
                  <div className="tw-flex-1 tw-min-w-0">
                    <div className="tw-flex tw-items-center tw-justify-between tw-gap-2">
                      <div className="tw-flex tw-flex-wrap tw-gap-2 tw-items-center tw-min-w-0">
                        ...
                        {memberName && memberName !== "—" && (
                          <span className="tw-bg-sky-50 tw-text-sky-700 tw-text-[11px] tw-font-medium tw-px-2 tw-py-[2px] tw-rounded-full">{memberName}</span>
                        )}
                        {vaccines.map((v, idx) => (
                          <span key={idx} className="tw-bg-emerald-50 tw-text-emerald-700 tw-text-[10px] tw-px-2 tw-py-[1px] tw-rounded-full">{v}</span>
                        ))}
                        {diseases.map((d, idx) => (
                          <span key={idx} className="tw-bg-amber-50 tw-text-amber-700 tw-text-[10px] tw-px-2 tw-py-[1px] tw-rounded-full">{d}</span>
                        ))}
                        {status && (
                      ...
                      <div className="tw-text-[10px] tw-text-slate-500 tw-text-right tw-whitespace-nowrap">
                    ...
                    <div className="tw-flex tw-gap-2 tw-mt-3">
                      ...
                      {!n.is_read && (
                        <button type="button" onClick={() => markRead(n.id)} className="tw-text-[11px] tw-bg-emerald-100 hover:tw-bg-emerald-200 tw-text-emerald-600 tw-rounded-lg tw-px-3 tw-py-1.5">
                          Đã đọc
                        </button>
                      )}
                    </div>
