...
export default function QuantityPillBooking({
  ...
  return (
    <div className={`tw-flex tw-items-center tw-justify-between tw-border tw-border-gray-300
                  ...
                  ${className}`} role="group" aria-label="Điều chỉnh số liều" >
      {/* nút trừ (xám) */}
      <button type="button" onClick={dec}  disabled={value <= min} aria-label="Giảm"
      ...
      {/* số */}
      <input  type="number"  inputMode="numeric" readOnly
        min={min} max={max} value={value}
        onChange={(e) => onChange(clamp(parseInt(e.target.value, 10)))}
        onBlur={(e) => onChange(clamp(parseInt(e.target.value, 10)))}
        onWheel={(e) => e.currentTarget.blur()}  // tránh cuộn chuột đổi số
        className="tw-w-14 tw-h-10  tw-flex tw-items-center tw-justify-center  tw-text-center tw-border-none tw-font-semibold 
                   tw-text-gray-800 focus:tw-outline-none tw-bg-transparent"
      />

      {/* nút cộng (gradient xanh) */}
      <button  type="button"  onClick={inc} disabled={value >= max} aria-label="Tăng"
        className={`tw-w-10 tw-h-10 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-transition-all
          ${value >= max  ? "tw-bg-gray-200 tw-text-gray-400 tw-cursor-not-allowed"
